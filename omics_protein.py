...
cc = pd.read_csv('protein-protein_interaction.csv')

# Build the no-directed graph
graph_cc = nx.Graph()

# -> add weighted edges
for j in range(len(cc['protein1'])):
    graph_cc.add_edge(cc['protein1'].iloc[j], cc['protein2'].iloc[j], weight=cc['combined_score'].iloc[j])
...
with open('modelpp.pickle', mode='wb') as f:
    pickle.dump(model, f)
