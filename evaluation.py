...
def main():
    ...
    for i in range(5):
        ...
        ecfp = np.load('./dataset/hard_dataset/cv_'+str(i)+'/test_fingerprint.npy')
        ecfp = np.asarray(ecfp, dtype='float32').reshape(-1,1024)
        file_interactions=np.load('./dataset/hard_dataset/'+'cv_'+str(i)+'/test_interaction.npy')
        print('Loading labels: test_interaction.npy')
        file_sequences=np.load('./dataset/hard_dataset/'+'cv_'+str(i)+'/test_reprotein.npy')
        print('Loading sequences: test_protein.npy')
        
        cID = np.load('./dataset/hard_dataset/'+'cv_'+str(i)+'/test_chemIDs.npy')
        print('Loading chemIDs: test_chemIDs.npy')
        with open('./dataset/hard_dataset/'+'cv_'+str(i)+'/test_proIDs.txt') as f:
        ...
        f = open(args.input+'/'+args.place+str(i)+'/evaluation_'+str(i)+'.csv', 'w') 
        ...
        for epoch in range(args.frequency, args.epoch+1 ,args.frequency):
            ...
            with cp.cuda.Device(args.gpu):
                serializers.load_npz(args.input+'/'+args.place+str(i)+'/model_snapshot_' + str(epoch), model)
